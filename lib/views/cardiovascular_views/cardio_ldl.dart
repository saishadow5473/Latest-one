...
class CardioLdl extends StatefulWidget {
  CardioLdl({Key key}) : super(key: key);

  @override
  _CardioLdlState createState() => _CardioLdlState();
}

class _CardioLdlState extends State<CardioLdl> {
  int low_density_lipoprotein = 70;
  bool mannual = true;
  ...
  void _initAsync() async {
    ...
    if(cardio_ldl.toString()!='null'||cardio_ldl.toString()!=''){
      setState(() {
        ldlController.text = cardio_ldl.toString();
      });
    }
  }
    ...
    return StreamBuilder<String>(
      builder: (context, snapshot) {
        return TextFormField(
          ...
          validator: (value) {
            if (value.isEmpty) {
              return 'low density lipoprotein\'s value can\'t be empty!';
          ...
          decoration: InputDecoration(
            ...
            suffixText: 'mg/dL',
            labelText: "low density lipoprotein (in mg/dL)",
            counterText: "",
    ...
    Widget _customButton() {
      return Container(
        ...
        child: GestureDetector(
          onTap: () {
            if (mannual == false) {
              SpUtil.putString('cardio_ldl', low_density_lipoprotein.toString());
              // Navigator.of(context).pushNamed(Routes.Aff);
              // Navigator.of(context).pushNamed(Routes.Spic);
              currentIndexOfCardio.value=6;
            } else {
              if (_formKey.currentState.validate()) {
                if (this.mounted) {
                  // setState(() {
                  SpUtil.putString('cardio_ldl', ldlController.text);
                  // Navigator.of(context).pushNamed(Routes.Aff);
                  // Navigator.of(context).pushNamed(Routes.Spic);
                  currentIndexOfCardio.value=6;
                  // });
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: <Widget>[
            ...
            Text(
              'Enter Your Low-density lipoprotein\'s value' ,
              textAlign: TextAlign.center,
              style: TextStyle(
