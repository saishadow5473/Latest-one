...
import 'package:ihl/views/JointAccount/create_new_account/create_dob.dart';
...
class CreateMob extends StatefulWidget {
  final Apirepository apiRepository;

  CreateMob({Key key, @required this.apiRepository})
      : assert(apiRepository != null),
        super(key: key);

  @override
  _CreateMobState createState() => _CreateMobState();
}

class _CreateMobState extends State<CreateMob> with TickerProviderStateMixin {
      ...
      builder: (context, snapshot) {
        return TextFormField(
          ...
          decoration: InputDecoration(
            ...
            prefixIcon: Padding(
              padding:
                  const EdgeInsetsDirectional.only(start: 5, top: 2, end: 6.0),
              child: Image.network(
        ...
        child: GestureDetector(
          onTap: () {
            ...
            if (mobController.text.isNotEmpty) {
                ...
                jUserExist().then((v) {
                  if (jUserExistR == false) {
                    if (this.mounted) {
                      setState(() {
                        SpUtil.putString(
                            'jMobile', mobController.text.toString());
                        isChecking = false;
                        //Navigator.of(context).pushNamed(Routes.Sdob);
                        SpUtil.putBool(SPKeys.jMobileGiven, true);

                        Get.to(CreateVerifyMobile(
                            mobileNumber: mobController.text.toString()));
                      });
                    }
                  }
                });
              ...
              SpUtil.putBool(SPKeys.jMobileGiven, false);
              Get.to(
                CreateDob(),
              );
            }
