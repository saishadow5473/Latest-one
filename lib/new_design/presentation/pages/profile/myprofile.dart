...
class _MyProfileState extends State<MyProfile> {
  ...
  loadData() async {
    ...
    dob.text = dobr ?? '';
    dob.text ??= '01/01/2000';
    // _selectedDate.value = convertToDate(dob.text);
    ...
    // if (height.text != '') {
    //   height.text = '${height.text} Cms';
    // }
    height.text ??= '';
    var weightr = res['User']['userInputWeightInKG'].toString();
    weight.text = weightr ?? '';
    // if (weight.text != '') {
    //   weight.text = '${weight.text} Kgs';
    // }
    weight.text ??= '';
    ...
    country.text = await getCountryFromPincode(pincode.text);
  }
  ...
  Future getPostalApi(var numb) async {
    final Dio dio = Dio();
    final response = await dio.get(
      'https://api.postalpincode.in/pincode/' + numb,
    );
    if (response.statusCode == 200) {
      print(response.runtimeType);
      var output = response.data;
      if (response.data != "" || response.data != null) {
        if (output[0]['PostOffice'] == null) {
          pincode.text = '600001';
          pincode.notifyListeners();
        } else {
          var cityTemp = output[0]['PostOffice'][0]['Pincode'].toString().replaceAll("Region", "");
          pincode.text = cityTemp == "" || cityTemp == null ? "" : cityTemp;
        }
      }
    }
  }
